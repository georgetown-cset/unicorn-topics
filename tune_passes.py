...
class TopicModel:
    ...
    def fit_lda_model(self):
                ...
                for b in beta:
                    for c in chunksize:
                        lda_model = gensim.models.LdaMulticore(corpus=corpus_set, id2word=self.id2word,
                                                               ...
                                                               passes=60, num_topics=num_topics, per_word_topics=True,
                                                               minimum_probability=0, eta=b)
                        if i == 1: # we only want to save the model if it's a model on the whole corpus
                            ...
                            with open(f"data/intermediate/passes_testing/lda_{num_topics}_"
                                      f"topics{b}_beta_{c}_chunksize.pkl", "wb") as file_out:
                        ...
                        print(f"Topic {num_topics}, beta {b} chunksize {c} corpus {corpus_titles[i]} "
        ...
        # coherence_model_lda = C
        # CoherenceModel(model=lda_model, texts=self.documents, dictionary=self.id2word,
        #                                  coherence='u_mass')
